...
.status-badge {
    display: inline-block; /* Ensures the status badge is on a new line */
}
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $user->name }}</td>
                                <td>{{ $user->username }}</td>
                                <td>{{ $businessProfile->mpesa_name }}</td>
                                <td>{{ $businessProfile->mpesa_no }}</td>
                                <td>{{ $totalShare }}</td>
                                <td>
                                    <div class="btn-group" role="group" aria-label="Basic example"></a>
                                        <span class="badge bg-primary" data-bs-toggle="modal" data-bs-target="#listModal{{ $user->id ."-". $firstShare->id }}">View list ({{ $pairedShare->count() }})</span>
                                    </div>
                                </td>
                                {{-- <td>
                                    @if($pairedWithIsAllPaid == 0)
                                ...
                                <td>
                                    ...
                                    @elseif(\Carbon\Carbon::parse($firstShare->pairedWithThis->created_at)->addHour(3) >= now() && $pairedWithIsAllPaid != 0)
                                    <div class="btn-group" role="group" aria-label="Basic example"></a>
                                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#paymentModal{{$user->id ."-". $firstShare->id }}">
                                            Pay now
                                        </button>
                                    </div>
                                    @else
